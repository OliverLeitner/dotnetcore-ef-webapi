...
using System.Text;
using DotNetCoreSampleApi.Models;
...
namespace JWTAuth.Controllers
{
    ...
    public class JWTAuthController : ControllerBase
    {
        ...
        private LoginModel AuthenticateUser(LoginModel login)
        {
            ...
            {
                var item = context.Tokens.FirstOrDefault(p => p.user_name.Contains(login.Username));

                if (item == null)
                    return null;
                else
                {
                    if (login.Username == item.user_name && login.EmailAddress == item.user_email)
                    {
                        user = new LoginModel { Username = login.Username, EmailAddress = login.EmailAddress };
                    }
                }
            }
